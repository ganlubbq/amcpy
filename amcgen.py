...
def mimo(sigIn,snr,NT,NR):
    "mimo    Adds MIMO channel effect to the signal"

    # Determine the dimension of input signal data
    N = sigIn.size  # Signal length
    K = int(N/NT)   # Stream length for each antenna

    # Generate channel matrix
    channelMatrix = np.random.randn(NT,NR) + np.random.randn(NT,NR)*1j

    # Create transmitting signal matrix
    st = np.zeros((NT,len(sigIn)/NT))

    # Split input signal to into equal length streams for different antenna
    for i in range(NT):
        st[i,:K] = sigIn[i*K:(i+1)*K]
